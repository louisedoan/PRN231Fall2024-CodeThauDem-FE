...
import { getLocations, createLocation, deleteLocation, updateLocation } from '../lib/api/Location';
            ...
            try {
                ...
                if (response.data) {
                    ...
                    if (updatedLoc.location && updatedLoc.flightRouteId) {
                        setLocations(locations.map(loc => loc.flightRouteId === updatedLoc.flightRouteId ? updatedLoc : loc));
                        setEditId(null);
                        setEditLocation('');
                        setMessage(response.message); // Set the message from the backend response
                        setMessageType(response.isSuccess ? 'success' : 'error'); // Set the message type based on the response
                    } else {
                        throw new Error('Invalid response data: location or flightRouteId is missing');
                    }
                } else {
                    console.error('Full response:', response); // Log the full response
                    throw new Error('Invalid response data: data is missing');
    ...
    const handleDeleteLocation = async (flightRouteId) => {
        ...
        try {
            ...
            if (response && response.data && response.data.isSuccess) {
                const updatedLocations = locations.filter(location => location.flightRouteId !== flightRouteId);
                ...
                <div className="w-full p-6 bg-white rounded-lg shadow-md max-w-full">
                    ...
                    <ul className="space-y-4 w-full">
                        {locations.map((location, index) => (
                            <li key={`${location.flightRouteId}-${index}`} className="flex justify-between items-center p-2 border border-gray-300 rounded-lg w-full">
